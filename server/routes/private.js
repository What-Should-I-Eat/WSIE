...
const UNABLE_TO_FAVORITE_UNEXPECTED_ERROR =
  "Error occurred trying to favorite recipe";
const SUCCESSFULLY_UNFAVORITE_RECIPE = "Successfully un-favorited recipe";
const UNABLE_TO_UNFAVORITE_UNEXPECTED_ERROR =
  "Error occurred trying to un-favorite recipe";
...
const UNABLE_TO_DELETE_RECIPE_ERROR =
  "Error occurred deleting user created recipe";
...
const SUCCESSFULLY_DELETED_RECIPE_REQUEST =
  "Successfully deleted recipe request";
const UNABLE_TO_DELETE_RECIPE_REQUEST_ERROR =
  "Error occurred deleting recipe publish request";
const UNABLE_TO_UPDATE_PUBLISH_RECIPE_ERROR =
  "Error occurred trying to update publish status";
...
const SUCCESSFULLY_UPDATED_ADMIN_STATE =
  "Successfully updated user admin capabilities";
const UNABLE_TO_UPDATE_REVIEW_STATUS_ERROR =
  "Error occurred trying to update review status";
...
privateRouter.post("/users/register", async (req, res) => {
  try {
    ...
    console.log(
      `Successfully created User: [${req.body.fullName}] with Username: [${userToLower}]`
    );
    res.json(savedUser);
  } catch (error) {
...
privateRouter.post("/users/find-username", async (req, res) => {
  try {
    ...
    if (
      (user.incorrectPasswordAttempts === 5 &&
        !hasTenMinutesPassed(user.incorrectPasswordAttemptTime)) ||
      user.incorrectPasswordAttempts >= 10
    ) {
      return res.status(user.incorrectPasswordAttempts >= 10 ? 453 : 452).json({
        error:
          user.incorrectPasswordAttempts >= 10
            ? "Must reset password"
            : "10 minute lockout",
    ...
    if (passwordValidated) {
      await User.updateOne(
        user,
        { $set: { incorrectPasswordAttempts: 0 } },
        { upsert: true, new: true }
      );
      ...
      await User.updateOne(
        ...
        {
          $set: {
            incorrectPasswordAttempts: updatedAttempts,
            incorrectPasswordAttemptTime: new Date(),
          },
        },
        { upsert: true, new: true }
...
privateRouter.post("/users/updateBio", async (req, res) => {
  try {
    ...
    if (bio.length > 500) {
      return res
        .status(400)
        .json({ error: "Bio exceeds character limit of 500" });
    }
...
privateRouter.get("/users/getVisibility", async (req, res) => {
  try {
...
privateRouter.post("/users/updateVisibility", async (req, res) => {
  try {
    const { username, isPublic } = req.body;

    if (!username || typeof isPublic !== "boolean") {
      return res.status(400).json({
        error: "Invalid request: Username and isPublic status are required",
      });
    }
...
privateRouter.put("/users/health", async (req, res) => {
  try {
    ...
    console.log(
      `Updated health for user: [${req.body.username.toLowerCase()}]`
    );
    res.json(user);
  } catch (error) {
...
privateRouter.post("/users/:id/favorites", async (req, res) => {
  try {
    ...
    const recipe = await Recipe.findOne({
      recipeName: req.body.favorites.recipeName,
    });
    if (!recipe) {
...
privateRouter.delete("/users/:id/favorites", async (req, res) => {
  try {
    ...
    const isRecipeFavoritedByOthers = await User.exists({
...
privateRouter.put("/users/profile/update_details", async (req, res) => {
  try {
    ...
    const updatedFullName = await User.updateOne(
      user,
      { $set: { fullName: `${req.body.firstName} ${req.body.lastName}` } },
      { upsert: true, new: true }
    );
    if (!updatedFullName) {
      return res
        .status(400)
        .json({ error: "Error occurred trying to update user details" });
    }
    ...
    res.status(500).json({
      error: "Internal server error occurred trying to update user details",
    });
  }
});
...
privateRouter.put("/users/profile/update_email", async (req, res) => {
  try {
    ...
    if (!updatedEmail) {
      return res
        .status(400)
        .json({ error: "Error occurred trying to update user email" });
    }
    ...
    res.status(500).json({
      error: "Internal server error occurred trying to update user email",
    });
  }
});
...
privateRouter.put("/users/profile/update_password", async (req, res) => {
  try {
    ...
    if (!passwordValidated) {
      return res.status(400).json({
        error:
          "Original password entered does not match existing. Failed to update user password",
      });
    }
    ...
    if (await validatePassword(user, req.body.newPassword)) {
      return res
        .status(400)
        .json({ error: "Passwords are identical - nothing to update" });
    }

    const hashedPassword = await bcrypt.hash(req.body.newPassword, 10);
    const updatedPassword = await User.updateOne(
      user,
      { $set: { password: hashedPassword, incorrectPasswordAttempts: 0 } },
      { upsert: true, new: true }
    );
    if (!updatedPassword) {
      return res
        .status(500)
        .json({ error: "Error occurred trying to update user password" });
    }
    ...
    res.status(500).json({
      error: "Internal server error occurred trying to update user password",
    });
  }
});

privateRouter.post(
  "/users/:id/recipe/create_recipe",
  upload.single("userRecipeImage"),
  async (req, res) => {
    try {
      const user = await User.findById(req.params.id);
      if (!user) {
...
privateRouter.get("/users/:id/recipe/get_recipe", async (req, res) => {
  try {
    ...
    const matchingRecipe = user.favorites.find(
      (recipe) =>
        recipe.recipeName === req.query.recipeName &&
        recipe.usernameCreator === user.username
    );
    if (!matchingRecipe) {
      return res
        .status(404)
        .json({ error: `[${req.query.recipeName}] not found` });
    }
    ...
    res
      .status(500)
      .json({ error: "Internal server error trying to get user recipe" });
  }
});
...
privateRouter.delete("/users/:id/recipe/delete_recipe", async (req, res) => {
  try {
    ...
    if (!recipe) {
      return res
        .status(404)
        .json({ error: `[${req.body.favorites.recipeName}] not found` });
    }
  ...
  try {
    ...
    if (savedRequest) {
      ...
      if (updatedRecipe) {
        res.status(200).json({
          success:
            "Successfully sent a request to publish the recipe. If accepted, your recipe will be published.",
        });
      } else {
        return res
          .status(500)
          .json({ error: "Error occurred trying to update pub request" });
      }
    } else {
      res
        .status(500)
        .json({ error: "Error occurred sending publish request message." });
    }
  } catch (error) {
    console.error(error);
    res
...
privateRouter.get("/recipes/get_requested_recipe", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal server error trying to get recipe" });
  }
});
...
privateRouter.put("/recipes/publish_review", async (req, res) => {
  try {
    ...
    const updatedIsPub = await Recipe.updateOne(
      recipe,
      {
        $set: {
          isPublished: req.body.favorites.isPublished,
          pubRequested: req.body.favorites.pubRequested,
          reported: false,
        },
      },
      { upsert: true, new: true }
    ...
    if (!updatedIsPub) {
      return res
        .status(400)
        .json({ error: "Error occurred trying to update publish status" });
    }
...
privateRouter.get("/recipes/get_pub_request", async (req, res) => {
  try {
    ...
    let recipePub = await RecipePubRequest.findOne({
      recipeId: recipeObjectId,
    });
    if (!recipePub) {
      return res
        .status(404)
        .json({ error: "Recipe publish request not found" });
    }
...
privateRouter.get("/recipes/publish_requests", async (_, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error occurred getting publish recipe requests" });
  }
});
...
privateRouter.post("/users/:id/recipe/post_review", async (req, res) => {
  try {
    ...
    if (!req.body.reviews) {
      return res
        .status(400)
        .json({ error: "Missing 'reviews' object in request body" });
    }
    const {
      reviewedRecipeId,
      writtenReview,
    ...
    if (!reviewedRecipeId || !writtenReview || !reviewerUsername) {
      ...
      return res.status(400).json({
        error:
          "Missing required fields: reviewedRecipeId, writtenReview, or reviewerUsername",
      });
    }
    let parentId = null;
    if (parentReviewId) {
    ...
    res
      .status(201)
      .json({ success: "Posted your review!", review: userReview });
  } catch (error) {
    console.error(error);
    res
      .status(500)
      .json({ error: "Error occurred sending post review message." });
  }
});
...
privateRouter.put("/recipes/report_recipe", async (req, res) => {
  try {
    ...
    const updated = await Recipe.updateOne(
      recipe,
      { $set: { isPublished: false, pubRequested: false, reported: true } },
      { upsert: true, new: true }
    );
    if (!updated) {
      return res
        .status(400)
        .json({ error: "Error occurred trying to report recipe" });
    }
...
privateRouter.put("/recipes/report_review", async (req, res) => {
  try {
    ...
    const updated = await RecipeReview.updateOne(
      recipeReview,
      { $set: { reviewReported: true } },
      { upsert: true, new: true }
    );
    if (!updated) {
      return res
        .status(400)
        .json({ error: "Error occurred trying to report review" });
    }
...
privateRouter.get("/recipes/get_reported_reviews", async (_, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal server error trying to get reviews" });
  }
});
...
function hasTenMinutesPassed(originalTimestamp) {
  const elapsedTimeInMilliseconds =
    new Date().getTime() - new Date(originalTimestamp).getTime();
