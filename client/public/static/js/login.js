$(document).ready(function () {
  ...
  if (username) {
    ...
    $("#publicProfileNavItem a").attr(
      "href",
      `/public_user_profile?username=${username}`
    );
  }
  ...
  $("#signUpForm").on("submit", async function (event) {
    ...
    const [firstName, lastName, email, username, password, retypedPassword] =
      formArray.map(({ value }) => value);
    const fullName = `${firstName} ${lastName}`;
    const validationCode = validationHandler.validateSignupInput(
      email,
      username,
      password,
      retypedPassword
      ...
      .then(async (response) => {
        if (response.ok) {
          ...
          $("#verifyAccountForm").prepend(
            '<div class="alert alert-warning">' + VERIFY_ACCOUNT + "</div>"
          );
        } else {
          const data = await response.json();
      ...
      .then(async (response) => {
        if (response.ok) {
          ...
          if (data.username) {
            ...
            $("#publicProfileNavItem a").attr(
              "href",
              `/public_user_profile?username=${data.username}`
            );
          }
  ...
  $(
    "#showSignInModalContentBtn, #signInSwitch, #signInSwitchVerification"
  ).click(function () {
    $(
      "#signUpModalContent, #verifyAccountModalContent, #forgotUsernamePasswordModalContent, #forgotUsernamePasswordVerifyAccountModalContent, #resetAccountModalContent"
    ).hide();
    $(
      "#signUpForm, #verifyAccountForm, #forgotUsernamePasswordForm, #forgotUsernamePasswordVerifyAccountForm, #resetAccountForm"
    )
      .get(0)
      .reset();
    utils.clearMessageFromAuthModal(authClassesToRemove);
  ...
  $("#showSignUpModalContentBtn, #signUpSwitch").click(function () {
    $(
      "#signInModalContent, #verifyAccountModalContent, #forgotUsernamePasswordModalContent, #forgotUsernamePasswordVerifyAccountModalContent, #resetAccountModalContent"
    ).hide();
    $(
      "#signInForm, #verifyAccountForm, #forgotUsernamePasswordForm, #forgotUsernamePasswordVerifyAccountForm, #resetAccountForm"
    )
      .get(0)
      .reset();
    utils.clearMessageFromAuthModal(authClassesToRemove);
  ...
  $("#signInForgotUsernamePasswordBtn, #signUpForgotUsernamePasswordBtn").click(
    function () {
      $(
        "#signInModalContent, #signUpModalContent, #verifyAccountModalContent, #forgotUsernamePasswordVerifyAccountModalContent, #resetAccountModalContent"
      ).hide();
      $(
        "#signInForm, #signUpForm, #verifyAccountForm, #forgotUsernamePasswordVerifyAccountForm, #resetAccountForm"
      )
        .get(0)
        .reset();
      utils.clearMessageFromAuthModal(authClassesToRemove);
